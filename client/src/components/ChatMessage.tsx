...
import { MESSAGE_SEEN, MESSAGE_SENT } from '../utils/types';
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, isSender, seen }) => {
  return (
    <div className={`flex ${isSender ? 'justify-end' : 'justify-start'} mb-2`}>
      ...
      >
        {isSender && (
          !seen ? <span className="absolute mt-2 bottom-1 right-1 text-xs text-gray-400">Sending..</span> : seen === MESSAGE_SENT ? <span className="absolute mt-2 bottom-1 right-1 text-xs text-gray-400">sent</span>
